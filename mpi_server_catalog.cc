    ...
    while (1){
        if (msg_queue[tail_idx].filled == true){
            ...
            if (strncmp(file_path, "PID\t", 4) == 0){
                ...
                struct stat64 sb;
                // call fstat once OPEN (because only newly created or modified open msg were sent)
                fstat64(fd, &sb);
                ...
                if (open_files.find(fd) != open_files.end()){
                    ...
                    // layout check only for the newly created file (as layout does not change)
...
void check_alive(){
    ...
    // once the file creation/update starts, start monitoring
    printf("escape\n");
    while (true){
        this_thread::sleep_for(chrono::seconds(5));

        // check if there is no file currently opened
        if (open_files.empty()){
            bool all_dead = true;
            // check the pids of the client application
